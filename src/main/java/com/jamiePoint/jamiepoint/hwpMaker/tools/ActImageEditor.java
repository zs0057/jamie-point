...
public class ActImageEditor {

	public static void ActImageToHwp(ControlTable curCtr, HWPFile hwpFile, UserInfoForm userInfoForm , int firstIdx, int secondIdx) {
		
		 Row seventhRow = curCtr.getRowList().get(3);
	     Cell seventhCell = seventhRow.getCellList().get(0);
	     String firstImgPath = "basic_hwp" + File.separator + "image" + File.separator + firstIdx + ".jpg";
	     userInfoForm.makeImage(userInfoForm.getActivities().get(firstIdx).getActivityImg(), firstImgPath);
	     
	     InsertImage(seventhCell, firstImgPath, hwpFile);
		
		
	    if(secondIdx != -1) {
	    	Row eightRow = curCtr.getRowList().get(5);
	        Cell eightCell = eightRow.getCellList().get(0);
	        String SecondImgPath = "basic_hwp" + File.separator + "image" + File.separator + firstIdx + ".jpg";
		    userInfoForm.makeImage(userInfoForm.getActivities().get(secondIdx).getActivityImg(), firstImgPath);
		        
		    InsertImage(eightCell, SecondImgPath, hwpFile);
	    }
	...
	private static void InsertImage(Cell seventhCell, String imgFilePath, HWPFile hwpFile) {
		 int streamIndex = hwpFile.getBinData().getEmbeddedBinaryDataList().size() + 1;
	        ...
	        Rectangle shapePosition = new Rectangle(0, 0, 160, 90);
