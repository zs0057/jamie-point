...
public class PsImageEditor {
	public static void PsImageToHwp(ControlTable curCtr, HWPFile hwpFile, UserInfoForm userInfoForm) {
		...
		for(PsInfo act : PsInfos) {
			if(act.getType().equals("백준 Bronze/Silver/Gold/Platinum/Diamond/Ruby")){
			     ...
			     String bojImgPath = "basic_hwp" + File.separator + "image" + File.separator + "Boj.jpg";
			     userInfoForm.makeImage(act.getPsImage(), bojImgPath);
			        
			     InsertImage(seventhCell, bojImgPath, hwpFile);
			}
			if(act.getType().equals("프로그래머스 1레벨/2레벨/3레벨")) {
		        Row eightRow = curCtr.getRowList().get(5);
		        Cell eightCell = eightRow.getCellList().get(0);
		        String proImgPath = "basic_hwp" + File.separator + "image" + File.separator + "Pro.jpg";
		        userInfoForm.makeImage(act.getPsImage(), proImgPath);
		        
		        
		        InsertImage(eightCell, proImgPath, hwpFile);
			}
		}
